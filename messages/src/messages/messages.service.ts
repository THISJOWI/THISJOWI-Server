...
export class MessagesService implements OnModuleInit {
  ...
  async create(createMessageDto: CreateMessageDto) {
    ...
    if (!conversationId || conversationId === 'new') {
      ...
      const existingConv = await this.createConversation(
  ...
  async findByUsers(senderId: string, recipientId: string) {
  ...
  async getMessagesByConversation(conversationId: string, userId: string) {
    const allMessages = await this.findByUserId(userId);
    return allMessages.filter((msg: any) => msg.conversationId === conversationId);
  }

  async getMessagesBetweenUsers(userId: string, recipientId: string) {
    const allMessages = await this.findByUserId(userId);
    return allMessages
      .filter((msg: any) =>
  ...
  async markAsRead(conversationId: string, userId: string) {
    try {
      ...
      if (!client) return { success: false, message: 'Client not ready' };

      const allMessages = await this.findByUserId(userId);
      const unreadMessages = allMessages.filter(
        (msg: any) => msg.conversationId === conversationId && msg.recipientId === userId && !msg.isRead,
      );
  ...
  async delete(id: string, userId: string, type: 'me' | 'everyone' = 'everyone') {
    try {
      const client = this.messageClient;
      if (!client) return { success: false, message: 'Client not ready' };
