...
public class UserService {
    ...
    public void deleteUserById(Long userId) {
        try {
            ...
            // Programmatically evict usersById_v2 cache entry if present
            var cacheById = cacheManager.getCache("usersById_v2");
            if (cacheById != null) {
                cacheById.evict(userId);
                log.debug("Evicted usersById_v2 cache for ID: {}", userId);
            }
    ...
    @Cacheable(cacheNames = "usersByEmail_v2", key = "#email")
    ...
    public User saveUser(User user) {
        try {
            ...
            // Also update usersById_v2 cache
            if (user.getId() != null) {
                var cache = cacheManager.getCache("usersById_v2");
                if (cache != null) {
                    cache.put(user.getId(), user);
                }
            }
            
            // Evict usersByEmail_v2 cache to ensure consistency
            var emailCache = cacheManager.getCache("usersByEmail_v2");
    ...
    @Cacheable(cacheNames = "usersById_v2", key = "#id")
    ...
    public void updateUserPassword(Long userId, String newEncodedPassword) {
        try {
            ...
            // Evict caches
            var userByIdCache = cacheManager.getCache("usersById_v2");
    ...
    /**
     * Get all LDAP users in the same organization
     * Used for showing contacts in messaging
     */
    public List<User> getLdapUsersByOrgId(java.util.UUID orgId) {
        return userRepository.findByOrgIdAndIsLdapUserTrue(orgId);
    }
